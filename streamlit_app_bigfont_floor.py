...
st.markdown(
    ...
    f"照顧及專業服務 應有額度：{care_amount:,} 元 → "
    f"比例額度：{care_amount_ratio:,} 元 ［{care_amount:,}×（{active_days}÷{month_days}）］ → "
    f"扣減額度：{care_amount_deduction:,} 元"
    f"</p>",
    unsafe_allow_html=True
)
st.markdown(
    f"<p style='font-size:22px; font-weight:bold;'>"
    f"交通接送服務 應有額度：{transport_amount:,} 元 → "
    f"比例額度：{transport_amount_ratio:,} 元 ［{transport_amount:,}×（{active_days}÷{month_days}）］ → "
    f"扣減額度：{transport_amount_deduction:,} 元"
    f"</p>",
    unsafe_allow_html=True
)
